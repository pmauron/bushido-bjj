  ...
  return (
    <div style={s.page}>
      ...
      <div style={{ fontSize: 12, color: C.textDim, marginBottom: 8 }}>
        {filtered.length} kids{overdueCount > 0 && filterActive !== "overdue" ? ` · ${overdueCount} overdue (${currentCycle})` : ""}
      </div>
          ...
          <div key={kid.id} style={{ ...s.card, opacity: kid.active ? 1 : 0.5, display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 11, marginTop: 2, display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                ...
                {!st.hasCurrent && kid.active && <span style={{ color: "#f44", fontWeight: 600, fontSize: 10, background: "#f4422a22", padding: "1px 5px", borderRadius: 4 }}>OVERDUE {currentCycle}</span>}
...
function ReportingScreen({ roster, assessments, config, onViewProfile, onScore, readOnly, isAdmin, loggedGym, selections, attendance }) {
  ...
  return (
    <div style={s.page}>
      ...
      {/* Overdue */}
      <h2 style={s.h2}>Overdue {"\u00b7"} {filterCycle} ({overdueKids.length})</h2>
      {overdueKids.length === 0 ? (
        <div style={{ ...s.card, textAlign: "center", color: C.green }}>✓ All active kids assessed for {filterCycle}</div>
      ) : (
