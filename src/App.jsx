...
<script>window.print();</script>
      ...
      {/* Attendance Stats */}
      {kid && <AttendanceStats kidId={kid.id} attendance={attendance} selections={safeSelections} config={config} />}
...
function ReportingScreen({ roster, assessments, config, onViewProfile, onScore, readOnly, isAdmin, loggedGym, selections, attendance }) {
  ...
  return (
    <div style={s.page}>
      ...
      {reportView === "team" && <CompetitionTeamView roster={roster} assessments={assessments} config={config} selections={safeSelections} attendance={attendance} filterCycle={filterCycle} filterGym={filterGym} onViewProfile={onViewProfile} />}
...
function AppInner() {
  ...
  const safeAssessments = useMemo(() => {
    ...
    return assessments.filter(a => a && a.id && a.kidId);
  }, [assessments]);
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", WebkitFontSmoothing: "antialiased" }}>
      ...
      {/* Content */}
      {tab === "roster" && <RosterScreen roster={roster} setRoster={setRoster} config={safeConfig} assessments={safeAssessments} onViewProfile={viewProfile} defaultGym={loggedGym} isAdmin={isAdmin} selections={safeSelections} attendance={safeAttendance} />}
      {tab === "classes" && <ClassesScreen roster={roster} attendance={safeAttendance} setAttendance={setAttendance} config={safeConfig} loggedGym={loggedGym} isAdmin={isAdmin} selections={safeSelections} />}
      {tab === "promotion" && <PromotionScreen roster={roster} setRoster={setRoster} attendance={safeAttendance} config={safeConfig} setConfig={setConfig} loggedCoach={loggedCoach} isCommunity={isCommunity} isAdmin={isAdmin} loggedGym={loggedGym} onViewProfile={viewProfile} />}
      ...
      {tab === "rankings" && <RankingsScreen roster={roster} assessments={safeAssessments} config={safeConfig} selections={safeSelections} setSelections={isCommunity ? () => {} : setSelections} readOnly={isCommunity} isAdmin={isAdmin} loggedGym={loggedGym} />}
      {tab === "reports" && <ReportingScreen roster={roster} assessments={safeAssessments} config={safeConfig} onViewProfile={viewProfile} onScore={isCommunity ? () => {} : (kidId) => { setTab("score"); }} readOnly={isCommunity} isAdmin={isAdmin} loggedGym={loggedGym} selections={safeSelections} attendance={safeAttendance} />}
      {tab === "profile" && <ProfileScreen roster={roster} assessments={safeAssessments} setAssessments={setAssessments} config={safeConfig} setConfig={setConfig} selectedKidId={selectedKidId} setSelectedKidId={setSelectedKidId} onEditAssessment={editAssessment} isCommunity={isCommunity} isAdmin={isAdmin} loggedGym={loggedGym} attendance={safeAttendance} selections={safeSelections} />}
