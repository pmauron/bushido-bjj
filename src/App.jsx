    ...
    <div>
      ...
      <div style={s.card}>
              ...
              return (
                <div key={slot.id} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", borderBottom: `1px solid ${C.border}22` }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 10, color: C.textDim }}>{slot.durationMin} min · {slot.coach} · ends {endTime}</div>
                  </div>
        ...
        <button style={{ ...s.btnSm, width: "100%", marginTop: 8, textAlign: "center" }}
          onClick={() => setEditSlot({ id: uid(), gym: schedGym, day: 1, time: "10:00", durationMin: 60, classTypeId: groupTypes[0]?.id || "", coach: coachName(config.coaches[0]) || "" })}>
          + Add class slot
        </button>
      </div>
...
function SettingsScreen({ config, setConfig, roster, assessments, setRoster, setAssessments, setSelections, setAttendance, isAdmin }) {
  ...
  const DEMO_GYM = "Demo gym";
  const DEMO_COACH = "Demo Coach";
  const DEMO_CLASS_TYPES = [
    {id:"demo-fund",name:"Kids Fundamentals",category:"group",color:"#4CAF50"},
    {id:"demo-adv",name:"Kids Advanced",category:"group",color:"#2196F3"},
    {id:"comp",name:"Competition Training",category:"competition",color:"#f44336"},
    {id:"pt",name:"Private / PT",category:"private",color:"#607D8B"},
  ];
  const DEMO_SCHEDULE = [
    {id:"ds01",gym:DEMO_GYM,day:1,time:"10:00",durationMin:60,classTypeId:"demo-fund",coach:DEMO_COACH},
    {id:"ds02",gym:DEMO_GYM,day:3,time:"10:00",durationMin:60,classTypeId:"demo-fund",coach:DEMO_COACH},
    {id:"ds03",gym:DEMO_GYM,day:5,time:"10:00",durationMin:60,classTypeId:"demo-fund",coach:DEMO_COACH},
    {id:"ds04",gym:DEMO_GYM,day:3,time:"12:00",durationMin:90,classTypeId:"demo-adv",coach:DEMO_COACH},
    {id:"ds05",gym:DEMO_GYM,day:6,time:"09:00",durationMin:60,classTypeId:"demo-fund",coach:DEMO_COACH},
    {id:"ds06",gym:DEMO_GYM,day:6,time:"10:30",durationMin:90,classTypeId:"demo-adv",coach:DEMO_COACH},
    {id:"ds07",gym:DEMO_GYM,day:3,time:"14:00",durationMin:90,classTypeId:"comp",coach:DEMO_COACH},
    {id:"ds08",gym:DEMO_GYM,day:6,time:"14:00",durationMin:90,classTypeId:"comp",coach:DEMO_COACH},
  ];
  const loadDemoData = () => {
    if (!confirmDemo) { setConfirmDemo(true); return; }
    setDemoStatus("Generating...");
    const NAMES = ["Leo M","Asher M","Timmy Z","Oliver W","Mia C","Lucas L","Emma L","Noah W","Sophia Y","Liam H","Ava Z","Ethan X","Isabella S","Mason Z","Charlotte L","James G","Amelia H","Benjamin Z","Harper D","Jack M","Ella T","Alexander F","Luna J","Daniel S","Chloe X","Henry P","Grace L","Samuel H","Lily Z","Max W","Zoe F","Hugo D","Camille B","Felix S","Hana T","Kai N","Yuki S","Ravi S","Priya P","Marco R","Sofia C","Oscar B","Freya A","Tom R","Poppy E"];
    const BELTS_D = ["White","Grey-White","Grey","Grey-Black","Yellow-White","Yellow","Yellow-Black"];
    const CRIT = ["Standup","Top Game","Bottom Game","Submission","Defense","Strength","Cardio","Mobility","Attendance","Attitude","Participation","Performance"];
    const GOALS = ["Master scissor sweep","Improve hip escape speed","Learn 2 new takedowns","Better guard retention","Compete in next tournament","Work on arm bar from mount","Improve cardio for 5-min rounds","Develop submission chain","Practice grip fighting","Stay calm under pressure","Learn back take from turtle","Improve passing to side control"];
    ...
    const now = new Date();
    const WR = { U8:{Light:[0,23],Medium:[23,28],Heavy:[28,999]}, U10:{Light:[0,28],Medium:[28,34],Heavy:[34,999]}, U12:{Light:[0,32],Medium:[32,40],Heavy:[40,999]}, U14:{Light:[0,38],Medium:[38,48],Heavy:[48,999]} };
    const ac = a => a<8?"U8":a<10?"U10":a<12?"U12":"U14";
    const wc = (w,c) => { const r=WR[c]; if(!r)return"Medium"; for(const[k,[lo,hi]] of Object.entries(r)){if(w>=lo&&w<hi)return k;} return"Heavy"; };
    const ageAt = (dob,d) => { const x=new Date(d),b=new Date(dob); let a=x.getFullYear()-b.getFullYear(); if(x.getMonth()<b.getMonth()||(x.getMonth()===b.getMonth()&&x.getDate()<b.getDate()))a--; return a; };
    // Roster
    const newRoster = NAMES.map((name, i) => {
      const id = `K${String(i+1).padStart(3,"0")}`;
      const bi = i<5?ri(3,6):i<15?ri(2,5):i<30?ri(1,4):ri(0,2);
      const belt = BELTS_D[Math.min(bi, BELTS_D.length-1)];
      const stripes = bi>=4?ri(2,4):bi>=2?ri(0,3):ri(0,2);
      const yr = ri(2012,2020);
      const dob = `${yr}-${String(ri(1,12)).padStart(2,"0")}-${String(ri(1,28)).padStart(2,"0")}`;
      const weight = yr<=2013?ri(38,55):yr<=2016?ri(25,42):yr<=2018?ri(20,35):ri(16,28);
      const active = i!==1&&i!==9&&i!==18&&i!==21&&i!==22&&i!==27&&i!==33;
      let ma; if(stripes>=4){ma=i%3===0?ri(10,14):ri(3,6);}else if(stripes>=2){ma=i%2===0?ri(3,8):ri(0,2);}else{ma=ri(1,5);}
      return { id, name, dob, belt, weight, gyms:[DEMO_GYM], active, stripes, lastPromotionDate:ds(ad(now,-ma*30)) };
    });
    // Assessments
    const newAssessments = [];
    const cyDates = { "2025 Q3":["2025-07-","2025-08-","2025-09-"], "2025 Q4":["2025-10-","2025-11-","2025-12-"], "2026 Q1":["2026-01-","2026-02-"] };
    newRoster.forEach((kid,i) => {
      if(!kid.active && Math.random()>0.3) return;
      const n = kid.active?(Math.random()>0.3?2:1):1;
      const uC = n===2?[pk(["2025 Q3","2025 Q4"]),"2026 Q1"]:["2026 Q1"];
      uC.forEach(cy => {
        const pre=pk(cyDates[cy]); const day=String(ri(1,28)).padStart(2,"0");
        const scores={}; const base=BELTS_D.indexOf(kid.belt);
        CRIT.forEach(c=>{scores[c]=Math.min(5,Math.max(1,base+ri(-1,2)));});
        newAssessments.push({id:uid(),date:pre+day,cycle:cy,coach:DEMO_COACH,kidId:kid.id,scores});
      });
    });
    // Selections
    const actKids = newRoster.filter(k=>k.active);
    const cKids = actKids.filter((_,i)=>i%3===0||i%5===0).slice(0,15);
    const newSel = {};
    cKids.forEach(kid => {
      const a=ageAt(kid.dob,ds(now)); const key=`2026 Q1|${ac(a)} · ${wc(kid.weight,ac(a))}`;
      if(!newSel[key])newSel[key]=[]; newSel[key].push(kid.id);
    });
    // Attendance — matches DEMO_SCHEDULE
    const newAtt = [];
    const sD=new Date("2025-12-01"), eD=new Date("2026-02-28");
    for(let d=new Date(sD);d<=eD;d=ad(d,1)){
      const dow=d.getDay(), dStr=ds(d);
      // Fundamentals: Mon, Wed, Fri, Sat
      if([1,3,5].includes(dow)){
        const rec={}; actKids.forEach(k=>{if(Math.random()<0.7)rec[k.id]="attend";}); 
        newAtt.push({key:`${dStr}|10:00|demo-fund|${DEMO_GYM}`,date:dStr,time:"10:00",classTypeId:"demo-fund",scheduleId:dow===1?"ds01":dow===3?"ds02":"ds03",gym:DEMO_GYM,coach:DEMO_COACH,durationMin:60,records:rec});
      }
      if(dow===6){
        const rec={}; actKids.forEach(k=>{if(Math.random()<0.65)rec[k.id]="attend";});
        newAtt.push({key:`${dStr}|09:00|demo-fund|${DEMO_GYM}`,date:dStr,time:"09:00",classTypeId:"demo-fund",scheduleId:"ds05",gym:DEMO_GYM,coach:DEMO_COACH,durationMin:60,records:rec});
      }
      // Advanced: Wed, Sat
      if([3,6].includes(dow)){
        const rec={}; actKids.filter((_,i)=>i<25).forEach(k=>{if(Math.random()<0.6)rec[k.id]="attend";});
        const sid=dow===3?"ds04":"ds06";
        newAtt.push({key:`${dStr}|${dow===3?"12:00":"10:30"}|demo-adv|${DEMO_GYM}`,date:dStr,time:dow===3?"12:00":"10:30",classTypeId:"demo-adv",scheduleId:sid,gym:DEMO_GYM,coach:DEMO_COACH,durationMin:90,records:rec});
      }
      // Competition: Wed, Sat
      if([3,6].includes(dow)){
        const rec={}; cKids.forEach(k=>{const r=Math.random();if(r<0.75)rec[k.id]="attend";else if(r<0.9)rec[k.id]="excused";});
        const sid=dow===3?"ds07":"ds08";
        newAtt.push({key:`${dStr}|14:00|comp|${DEMO_GYM}`,date:dStr,time:"14:00",classTypeId:"comp",scheduleId:sid,gym:DEMO_GYM,coach:DEMO_COACH,durationMin:90,records:rec});
      }
    }
    // Goals & Promotion Log
    const newGoals = {};
    actKids.slice(0,20).forEach(kid=>{
      const n=ri(1,3); newGoals[kid.id]=[];
      for(let j=0;j<n;j++){newGoals[kid.id].push({text:GOALS[(actKids.indexOf(kid)*3+j)%GOALS.length],done:Math.random()<0.3,date:ds(ad(now,-ri(5,60)))});}
    });
    const newPLog = [];
    newRoster.filter(k=>k.active&&BELTS_D.indexOf(k.belt)>=2).slice(0,12).forEach(kid=>{
      const bi=BELTS_D.indexOf(kid.belt);
      if(bi>=1) newPLog.push({kidId:kid.id,type:"belt",from:BELTS_D[bi-1],to:kid.belt,date:ds(ad(now,-ri(200,400))),by:DEMO_COACH});
      for(let s=1;s<=Math.min(kid.stripes,4);s++) newPLog.push({kidId:kid.id,type:"stripe",from:s-1,to:s,belt:kid.belt,date:ds(ad(now,-ri(10,180))),by:DEMO_COACH});
    });
    newPLog.sort((a,b)=>a.date.localeCompare(b.date));
    // Apply
    setRoster(newRoster);
    setAssessments(newAssessments);
    setSelections(newSel);
    setAttendance(newAtt);
    setConfig(prev => ({ ...prev, goals: newGoals, promotionLog: newPLog, classTypes: DEMO_CLASS_TYPES, weeklySchedule: DEMO_SCHEDULE }));
    setConfirmDemo(false);
    setDemoStatus(`✅ ${newRoster.length} demo kids loaded · Jing'An & Xuhui empty`);
    ...
    <div style={s.page}>
      ...
      {section === "reset" && (
        <div>
          ...
          <div style={s.card}>
            <p style={{ fontWeight: 600, marginBottom: 4 }}>🔄 Reload Demo Data</p>
            <p style={{ color: C.textDim, fontSize: 13, marginBottom: 12 }}>Populate Demo gym with 45 mock kids, assessments, attendance, promotions, and goals. Jing'An and Xuhui will be emptied. Settings are preserved.</p>
            <button style={{ ...s.btn, background: confirmDemo ? "#ff9800" : "#2196F3" }} onClick={loadDemoData}>{confirmDemo ? "⚠ TAP AGAIN TO CONFIRM" : "🔄 Reload Demo Data"}</button>
            {demoStatus && <div style={{ marginTop: 8, fontSize: 13, color: demoStatus.startsWith("✅") ? C.accent : "#ff9800" }}>{demoStatus}</div>}
          </div>
                ...
                return (
                  <div key={sl.id} onClick={() => goToRecord(sl)} style={{
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 11, color: C.textDim }}>{sl.coach} · {sl.durationMin} min</div>
                    </div>
          ...
          <div key={card.recKey}>
            <div style={{ ...s.card, borderLeft: `3px solid ${catColor}`, cursor: "pointer" }} onClick={() => { setExpanded(isExpanded ? null : card.recKey); setSearch(""); setSearchCross(""); }}>
              ...
              <div style={{ fontSize: 11, color: C.textDim }}>
                {card.type === "scheduled" ? `Coach: ${card.slot.coach}` : card.record?.coach ? `Coach: ${card.record.coach}` : ""} · {totalKids} kids
                {isMarked ? (
                  <b style={{ color: C.green, marginLeft: 6 }}>✓ {card.attendCount}/{totalKids} marked</b>
                ) : (
                  <span style={{ color: C.orange, marginLeft: 6 }}>⏳ Not yet marked</span>
                )}
...
function AppInner() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", WebkitFontSmoothing: "antialiased" }}>
      ...
      {tab === "classes" && <ClassesScreen roster={roster} attendance={attendance || []} setAttendance={setAttendance} config={safeConfig} loggedGym={loggedGym} isAdmin={isAdmin} selections={selections} />}
