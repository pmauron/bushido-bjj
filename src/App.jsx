...
function ReportingScreen({ roster, assessments, config, onViewProfile, onScore, readOnly, isAdmin, loggedGym, selections, attendance }) {
  ...
  const OverviewTab = () => {
    ...
    const totalAvgWeekly = totalActive ? allActive.reduce((s2, k) => s2 + kidWeeklyAvg(k.id, null), 0) / totalActive : 0;
    ...
    const totalAssessed = allActive.filter(k => assessedIds.has(k.id)).length;
    const totalAtRisk = allActive.filter(k => kidWeeklyAvg(k.id, null) < 1).length;

    // Weekly trend (academy-wide, 12 weeks)
    const now = new Date();
    const weeklyTrend = [];
    for (let w = 0; w < 12; w++) {
      ...
      const weekRecs = att90.filter(r => r.date > startStr && r.date <= endStr);
  ...
  const RetentionTab = () => {
    ...
    const attGym = gym ? att90.filter(r => r.gym === gym) : att90;
    ...
    // ── 🔴 Gone Cold: no attendance in 14+ days, had at least 1 class ever ──
    const goneCold = kids.filter(k => {
      const la = lastAtt[k.id];
      return la && la < cutoff14;
    }).map(k => ({ kid: k, lastDate: lastAtt[k.id], days: daysSince(lastAtt[k.id]) }))
      .sort((a, b) => b.days - a.days);

    // ── 🟡 Cooling Off: 2+/wk in prev month → <1/wk this month ──
    const coolingOff = kids.filter(k => {
      if (goneCold.some(g => g.kid.id === k.id)) return false; // already in cold
      ...
      const prevAvg = prevRecs / 4; // ~4 weeks
      const currAvg = currRecs / 4;
      return prevAvg >= 2 && currAvg < 1;
    }).map(k => {
      const prevRecs = (attendance || []).filter(r => r.date >= cutoffPrev && r.date < cutoff30 && (!gym || r.gym === gym) && r.records?.[k.id] === "attend").length;
      const currRecs = (attendance || []).filter(r => r.date >= cutoff30 && (!gym || r.gym === gym) && r.records?.[k.id] === "attend").length;
    ...
    // ── 🟢 New & Fragile: joined/first attendance in last 60 days, <2/wk ──
    const newFragile = kids.filter(k => {
      if (goneCold.some(g => g.kid.id === k.id)) return false;
      if (coolingOff.some(g => g.kid.id === k.id)) return false;
      const firstAtt = (attendance || []).filter(r => (!gym || r.gym === gym) && r.records?.[k.id] === "attend").map(r => r.date).sort()[0];
      const isNew = (k.joinDate && k.joinDate >= cutoff60) || (firstAtt && firstAtt >= cutoff60);
    ...
    return (
      <>
        ...
        {goneCold.length === 0 ? (
          ...
          <div style={s.card}>
            <div style={{ fontSize: 10, color: C.textDim, marginBottom: 8 }}>No attendance in 14+ days — reach out now</div>
            {goneCold.map(g => (
        ...
        {coolingOff.length === 0 ? (
          ...
          <div style={s.card}>
            <div style={{ fontSize: 10, color: C.textDim, marginBottom: 8 }}>Was 2+/wk last month → &lt;1/wk now — check in before they drift</div>
            {coolingOff.map(g => (
              <KidRow key={g.kid.id} kid={g.kid} onClick={() => onViewProfile(g.kid.id)}
        ...
        {newFragile.length === 0 ? (
          ...
          <div style={s.card}>
            <div style={{ fontSize: 10, color: C.textDim, marginBottom: 8 }}>Joined in last 60 days, attending &lt;2/wk — hasn't built the habit yet</div>
            {newFragile.map(g => (
              <KidRow key={g.kid.id} kid={g.kid} onClick={() => onViewProfile(g.kid.id)}
