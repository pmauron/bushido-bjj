...
function computePromoProjection(kid, attendance, config) {
  ...
  // Determine next milestone: stripe or belt
  const needsBeltPromo = hasNextBelt && stripes >= maxStripes;
  const isAtMaxBelt = !hasNextBelt;

  let type, gates = [], projectedDate = null;

  if (isAtMaxBelt && stripes >= maxStripes) {
    return { type: "complete", gates: [], projectedDate: null, weeklyAvg, classesDone, lastPromo, stripes, nextBelt: null };
  }

  if (needsBeltPromo) {
    // Belt promotion: 3 gates
    type = "belt";
    ...
    gates = [
      ...
      { label: "Time", labelZh: "时间", current: months, required: rules.beltMonths || 9, unit: "mo", done: monthsNeeded <= 0 },
      { label: "Stripes", labelZh: "条纹", current: stripes, required: maxStripes, done: true },
    ];
    const dates = [classDate, monthDate].filter(Boolean);
    projectedDate = dates.length > 0 ? dates.sort().pop() : null;
  } else {
    // Next stripe
    type = "stripe";
    const classesNeeded = Math.max(0, (rules.stripeClasses || 10) - classesDone);
    const classDate = weeklyAvg > 0 && classesNeeded > 0 ? addWeeks(classesNeeded / weeklyAvg) : (classesNeeded <= 0 ? today() : null);

    gates = [
      { label: "Classes", labelZh: "课时", current: classesDone, required: rules.stripeClasses || 10, done: classesNeeded <= 0 },
    ];
    projectedDate = classDate;
  }

  return { type, gates, projectedDate, weeklyAvg, classesDone, lastPromo, stripes, nextBelt, maxStripes };
}
...
function PromotionProgress({ kid, attendance, config, setConfig, readOnly }) {
  ...
  if (promo.type === "complete") {
    return (
      <>
        <h2 style={s.h2}>Next Promotion 下次晋级</h2>
  ...
  const typeLabel = promo.type === "belt" ? `🥋 Belt → ${promo.nextBelt}` : `🎖 Stripe ${(promo.stripes || 0) + 1}`;
...
${(() => {
  const promoProj = computePromoProjection(kid, attendance, config);
  if (promoProj.type === "complete") return "";
  const targetDt = (config.promoTargets || {})[kid.id] || "";
  const displayDt = targetDt || promoProj.projectedDate;
  const isOverride = targetDt && promoProj.projectedDate && targetDt !== promoProj.projectedDate;
  const typeStr = promoProj.type === "belt" ? `🥋 Belt Promotion → ${promoProj.nextBelt}` : `🎖 Next Stripe (${(promoProj.stripes || 0) + 1})`;
  return `<div class="section">
  <h2>Next Promotion 下次晋级</h2>
  <div style="padding:12px 16px;background:#f9f9f9;border-radius:8px;border-left:3px solid #C41E3A;margin-bottom:12px">
    <div style="font-weight:700;font-size:14px;margin-bottom:8px">${typeStr}</div>
    ${promoProj.gates.map(g => {
      const pct = Math.min(100, g.required > 0 ? (g.current / g.required) * 100 : 100);
      const color = g.done ? "#4CAF50" : "#FF9800";
      return `<div style="margin-bottom:8px">
        <div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:3px">
          <span style="color:#666">${g.label} ${g.labelZh}</span>
          <span style="font-weight:700;color:${g.done ? '#4CAF50' : '#333'}">${g.current}${g.unit || ''} / ${g.required}${g.unit || ''} ${g.done ? '✓' : ''}</span>
        </div>
    ...
    <div style="margin-top:12px;padding:8px 12px;background:#fff;border-radius:6px;display:flex;justify-content:space-between;align-items:center">
      <div>
        <div style="font-size:10px;color:#888;text-transform:uppercase;letter-spacing:0.5px">${targetDt ? 'Target Date 目标日期' : 'Projected Date 预计日期'}</div>
        <div style="font-size:18px;font-weight:800;color:#C41E3A">${displayDt || 'Insufficient data'}</div>
        ${isOverride ? `<div style="font-size:10px;color:#999;margin-top:2px">Projected 预计: ${promoProj.projectedDate}</div>` : ''}
      </div>
      <div style="font-size:11px;color:#888">${promoProj.weeklyAvg > 0 ? promoProj.weeklyAvg.toFixed(1) + ' classes/wk' : ''}</div>
    </div>
  </div>
</div>`;
})()}
...
function AppInner() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", WebkitFontSmoothing: "antialiased" }}>
      ...
      {/* Content */}
      {tab === "roster" && <RosterScreen roster={roster} setRoster={setRoster} config={safeConfig} assessments={safeAssessments} onViewProfile={viewProfile} defaultGym={loggedGym} isAdmin={canToggleGyms} selections={safeSelections} attendance={safeAttendance} />}
