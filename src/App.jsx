  ...
  const filtered = useMemo(() => {
    return roster.filter(k => {
      ...
      switch (sortBy) {
        ...
        case "age": return ageAt(a.dob, today()) - ageAt(b.dob, today());
        case "weight": return (a.weight || 0) - (b.weight || 0);
        case "score_desc": return (kidStatus[b.id]?.score || 0) - (kidStatus[a.id]?.score || 0);
        case "score_asc": return (kidStatus[a.id]?.score || 0) - (kidStatus[b.id]?.score || 0);
        case "training_desc": return (weeklyAvg[b.id] || 0) - (weeklyAvg[a.id] || 0);
        case "training_asc": return (weeklyAvg[a.id] || 0) - (weeklyAvg[b.id] || 0);
        case "belt": {
          const bi = (config.belts || []).indexOf(a.belt) - (config.belts || []).indexOf(b.belt);
          return bi !== 0 ? bi : (b.stripes || 0) - (a.stripes || 0);
        }
        default: return 0;
      }
    });
      ...
      {showFilters && (
        <div style={{ ...s.card, marginBottom: 12, padding: 12, border: `1px solid ${C.border}` }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            ...
            <div>
              <div style={{ fontSize: 10, color: C.textDim, textTransform: "uppercase", marginBottom: 3, fontWeight: 700 }}>Sort by</div>
              <select style={{ ...s.select, fontSize: 12 }} value={sortBy} onChange={e => setSortBy(e.target.value)}>
                <option value="name">Name A→Z</option>
                <option value="age">Age (youngest)</option>
                <option value="weight">Weight (lightest)</option>
                <option value="belt">Belt (highest)</option>
                <option value="score_desc">Score (highest)</option>
                <option value="score_asc">Score (lowest)</option>
                <option value="training_desc">Training (most)</option>
                <option value="training_asc">Training (least)</option>
              </select>
            </div>
          </div>
          {hasFilters && (
