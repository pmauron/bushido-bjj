...
function AttendanceStats({ kidId, attendance, selections }) {
  ...
  const stats = useMemo(() => {
    ...
    const avgWeeklyQ = (groupQAttend / weeksQ).toFixed(1);
    const avgWeekly90 = (group90Attend / weeks90).toFixed(1);

    // Competition classes (only for selected kids)
    let compStats = null;
    if (isCompKid) {
    ...
    return {
      ...
      compStats,
      hasData: group90Total > 0 || (compStats && compStats.d90Total > 0),
    };
  ...
  return (
    <>
      ...
      <div style={s.card}>
        ...
        <MetricRow label="Avg weekly (quarter)" value={stats.avgWeeklyQ} sub="classes/week" color={weeklyColor(stats.avgWeeklyQ)} />
        <MetricRow label="Avg weekly (90 days)" value={stats.avgWeekly90} sub="classes/week" color={weeklyColor(stats.avgWeekly90)} />
        <MetricRow label="Total this quarter" value={stats.groupQAttend} sub="classes" />
        <MetricRow label="Total last 90 days" value={`${stats.group90Attend}/${stats.group90Total}`} sub="attended" />
...
function CompetitionTeamView({ roster, assessments, config, selections, attendance, filterCycle, filterGym, onViewProfile }) {
  ...
  // Attendance: avg weekly group + competition class %
  ...
  const kidAttendance = (kidId) => {
    const groupDays = att90.filter(r => r.type === "group");
    const groupAttend = groupDays.filter(r => r.records?.[kidId] === "attend").length;
    const avgWeekly = parseFloat((groupAttend / weeks90).toFixed(1));
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8, marginBottom: 16 }}>
        {[
          ...
          { label: "Avg Weekly", value: avgWeekly.toFixed(1), color: avgWeekly >= 3 ? "#4CAF50" : avgWeekly >= 2 ? "#ff9800" : C.red },
        ].map((c, i) => (
