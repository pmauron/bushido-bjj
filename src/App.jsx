...
/* ━━━ ROSTER SCREEN ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
function RosterScreen({ roster, setRoster, config, assessments, onViewProfile, defaultGym }) {
  ...
  return (
    <div style={s.page}>
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal === "add" ? "Add Kid" : "Edit Kid"}>
        {modal && <KidForm kid={modal === "add" ? { id: nextId(), name: "", dob: "", belt: "White", weight: 25, gyms: [defaultGym || config.gyms[0]], active: true } : modal} config={config} onSave={saveKid} onCancel={() => setModal(null)} />}
      </Modal>
    </div>
  );
}
...
function AttendanceStats({ kidId, attendance, selections }) {
  ...
  const stats = useMemo(() => {
    ...
    const compute = (type) => {
      // All class days of this type
      const allDays = (attendance || []).filter(r => r.type === type);
      const thisMonthDays = allDays.filter(r => r.date?.startsWith(thisMonth));
      const quarterDays = allDays.filter(r => r.date >= qStart);
      const last90Days = allDays.filter(r => r.date >= cutoff90);

      const count = (days) => {
        let attend = 0, excused = 0, absent = 0;
        days.forEach(r => {
          const st = r.records?.[kidId];
          if (st === "attend") attend++;
          else if (st === "excused") excused++;
          else absent++;
        });
        return { attend, excused, absent, total: days.length };
      };

      const monthly = count(thisMonthDays);
      const quarter = count(quarterDays);
      const last90 = count(last90Days);

      const monthsInQ = now.getMonth() % 3 + 1;
      const qAvg = monthsInQ > 0 ? (quarter.attend / monthsInQ).toFixed(1) : "0";

      return { monthly, quarter, last90, qAvg, hasData: last90.total > 0 };
    };

    return { group: compute("group"), competition: isCompKid ? compute("competition") : null };
  }, [attendance, kidId, isCompKid]);

  if (!stats.group.hasData && !stats.competition?.hasData) return null;
    ...
    return (
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0", borderBottom: `1px solid ${C.border}` }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11 }}>
          <span style={{ color: C.green, fontWeight: 700 }}>{data.attend}✅</span>
          <span style={{ color: C.orange }}>{data.excused}🟡</span>
          <span style={{ color: C.textDim }}>{data.absent} absent</span>
          <span style={{ color: C.text, fontWeight: 600, fontSize: 10 }}>/ {data.total}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <>
      ...
      <div style={s.card}>
        <div style={{ fontSize: 12, fontWeight: 700, color: C.text, marginBottom: 6 }}>👥 Group Classes</div>
        <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${C.border}` }}>
          <span style={{ fontSize: 12, color: C.textDim }}>This month</span>
          <span style={{ fontSize: 16, fontWeight: 800, color: C.green }}>{stats.group.monthly.attend}</span>
        </div>
        <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${C.border}` }}>
          <span style={{ fontSize: 12, color: C.textDim }}>Avg per month (quarter)</span>
          <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{stats.group.qAvg}</span>
        </div>
        <StatRow label="This quarter" data={stats.group.quarter} />
        <StatRow label="Last 90 days" data={stats.group.last90} />

        {stats.competition && stats.competition.hasData && (
          <>
            <div style={{ fontSize: 12, fontWeight: 700, color: C.red, marginTop: 14, marginBottom: 6 }}>🏆 Competition Classes</div>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${C.border}` }}>
              <span style={{ fontSize: 12, color: C.textDim }}>This month</span>
              <span style={{ fontSize: 16, fontWeight: 800, color: C.green }}>{stats.competition.monthly.attend}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "6px 0", borderBottom: `1px solid ${C.border}` }}>
              <span style={{ fontSize: 12, color: C.textDim }}>Avg per month (quarter)</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>{stats.competition.qAvg}</span>
            </div>
            <StatRow label="This quarter" data={stats.competition.quarter} />
            <StatRow label="Last 90 days" data={stats.competition.last90} />
          </>
        )}
      </div>
    </>
...
function ProfileKidSelector({ roster, selectedKidId, setSelectedKidId }) {
  const [search, setSearch] = useState("");
  const activeKids = roster.filter(k => k.active).sort((a, b) => a.name.localeCompare(b.name));
  const filtered = search ? activeKids.filter(k => k.name.toLowerCase().includes(search.toLowerCase())) : [];
  const showList = search.length > 0 && filtered.length > 0 && !activeKids.find(k => k.id === selectedKidId && k.name.toLowerCase() === search.toLowerCase());
...
function ProfileScreen({ roster, assessments, setAssessments, config, setConfig, selectedKidId, setSelectedKidId, onEditAssessment, isCommunity, attendance, selections }) {
...
<script>window.print();</script>
      ...
      <ProfileKidSelector roster={roster} selectedKidId={selectedKidId} setSelectedKidId={setSelectedKidId} />
...
/* ━━━ LOGIN SCREEN ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
/* ━━━ PROMOTION SCREEN ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
function PromotionScreen({ roster, setRoster, attendance, config, setConfig, loggedCoach, isCommunity, onViewProfile }) {
  ...
  // Count classes since a given date for a kid (both group + competition, only "attend")
  ...
  // Build eligibility for each active kid
  const eligibility = useMemo(() => {
    return roster.filter(k => k.active).map(kid => {
      ...
      // Belt-ready first, then stripe-ready, then by classes desc
      if (a.beltReady !== b.beltReady) return b.beltReady - a.beltReady;
      if (a.stripeReady !== b.stripeReady) return b.stripeReady - a.stripeReady;
      return b.classes - a.classes;
    });
  }, [roster, attendance, config, rules]);
  ...
  const readyCount = eligibility.filter(e => e.stripeReady || e.beltReady).length;
    ...
    <div style={{ display: "flex", gap: 3 }}>
      {Array.from({ length: max }).map((_, i) => (
        <div key={i} style={{
          ...
          background: i < count ? C.red : C.border,
        }} />
      ))}
    </div>
  );
  ...
  return (
    <div style={s.page}>
      <h1 style={{ ...s.h1, marginBottom: 4 }}>Promotions</h1>
      ...
      {readyCount > 0 && (
        <div style={{ ...s.card, background: C.green + "11", borderColor: C.green, marginBottom: 12 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: C.green }}>🎉 {readyCount} kid{readyCount > 1 ? "s" : ""} eligible for promotion</div>
        </div>
      )}

      {eligibility.map(e => {
        const { kid, stripes, classes, months, stripeReady, beltReady } = e;
        const stripeProgress = Math.min(classes / (rules.stripeClasses || 10), 1);
        const isEligible = stripeReady || beltReady;

        return (
          <div key={kid.id} style={{
            ...s.card, marginBottom: 8,
            border: isEligible ? `1px solid ${beltReady ? C.orange : C.green}` : `1px solid ${C.border}`,
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div style={{ flex: 1 }} onClick={() => onViewProfile(kid.id)}>
                <div style={{ fontWeight: 700, fontSize: 14, color: C.text, cursor: "pointer" }}>{kid.name}</div>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
                  <BeltBadge belt={kid.belt} />
                  <StripeDots count={stripes} max={rules.stripesForBelt || 4} />
                </div>
              </div>
              <div style={{ textAlign: "right" }}>
                <div style={{ fontSize: 18, fontWeight: 800, color: C.text }}>{classes}</div>
                <div style={{ fontSize: 9, color: C.textDim }}>classes</div>
              </div>
            </div>

            {/* Progress bar to next stripe */}
            {stripes < (rules.stripesForBelt || 4) && !beltReady && (
              <div style={{ marginTop: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: C.textDim, marginBottom: 3 }}>
                  <span>Next stripe: {classes}/{rules.stripeClasses || 10} classes</span>
                  <span>{(stripeProgress * 100).toFixed(0)}%</span>
                </div>
                <div style={{ height: 4, background: C.border, borderRadius: 2, overflow: "hidden" }}>
                  <div style={{ height: "100%", width: `${stripeProgress * 100}%`, background: stripeReady ? C.green : C.red, borderRadius: 2, transition: "width 0.3s" }} />
                </div>
              </div>
            )}

            {/* Belt progress */}
            {stripes >= (rules.stripesForBelt || 4) && e.hasNextBelt && (
              <div style={{ marginTop: 8, fontSize: 10, color: C.textDim }}>
                Belt: {classes}/{rules.beltClasses || 40} classes · {months}/{rules.beltMonths || 9} months
                {classes >= (rules.beltClasses || 40) && <span style={{ color: C.green }}> ✓ classes</span>}
                {months >= (rules.beltMonths || 9) && <span style={{ color: C.green }}> ✓ time</span>}
              </div>
            )}

            {/* Action buttons */}
            {(stripeReady || beltReady) && (
              <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
                {stripeReady && !beltReady && (
                  <button style={{ ...s.btn, flex: 1, background: C.green }} onClick={() => awardStripe(kid)}>
                    🎖 Award Stripe {stripes + 1}
                  </button>
                )}
                {beltReady && (
                  <button style={{ ...s.btn, flex: 1, background: C.orange }} onClick={() => awardBelt(kid)}>
            ...
            {!e.hasNextBelt && stripes >= (rules.stripesForBelt || 4) && (
              <div style={{ marginTop: 8, fontSize: 11, color: C.textDim, fontStyle: "italic" }}>Highest belt reached</div>
            )}
          </div>
        );
      })}
      ...
      {eligibility.length === 0 && (
        <div style={{ ...s.card, textAlign: "center", color: C.textDim }}>No active kids in roster</div>
      )}

      {/* Recent Promotions */}
      {(config.promotionLog || []).length > 0 && (
        <>
          <h2 style={s.h2}>Recent Promotions</h2>
          {[...(config.promotionLog || [])].reverse().slice(0, 10).map((p, i) => {
...
function AttendanceScreen({ roster, attendance, setAttendance, config, loggedGym, isAdmin, selections }) {
  ...
  const cycleStatus = (kidId) => {
    ...
    // null (absent) → attend → excused → null (absent)
    const order = [null, "attend", "excused"];
  ...
  return (
    <div style={s.page}>
      <h1 style={{ ...s.h1, marginBottom: 12 }}>Attendance</h1>
        ...
        <>
          <div style={{ ...s.card, marginBottom: 10 }}>
            <div style={{ display: "flex", gap: 12, fontSize: 12, fontWeight: 600 }}>
              ...
              <span style={{ color: C.orange }}>🟡 {counts.excused} excused</span>
              <span style={{ color: C.textDim }}>— {counts.absent} absent</span>
            </div>
          </div>

          <div style={{ fontSize: 10, color: C.textDim, marginBottom: 6, textAlign: "center" }}>Tap to cycle: — Absent → ✅ Present → 🟡 Excused</div>

          {/* Home gym kids */}
          <div style={{ border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden" }}>
            {homeKids.map((k, i) => renderKid(k, i, i === homeKids.length - 1 && crossKids.length === 0))}
          </div>

          {/* Cross-training kids */}
          {crossKids.length > 0 && (
            <>
              <div style={{ fontSize: 11, color: C.textDim, fontWeight: 700, marginTop: 12, marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.5 }}>
                Cross-training ({crossKids.length})
              </div>
              <div style={{ border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden" }}>
                {crossKids.map((k, i) => renderKid(k, i, i === crossKids.length - 1))}
              </div>
            </>
          )}
        </>
...
const NAV_ITEMS = [
  ...
  { key: "promotion", icon: "🎖", label: "Promo" },
...
export default function App() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", WebkitFontSmoothing: "antialiased" }}>
      ...
      {/* Content */}
      {tab === "roster" && <RosterScreen roster={roster} setRoster={setRoster} config={safeConfig} assessments={safeAssessments} onViewProfile={viewProfile} defaultGym={loggedGym} />}
      ...
      {tab === "promotion" && <PromotionScreen roster={roster} setRoster={setRoster} attendance={attendance || []} config={safeConfig} setConfig={setConfig} loggedCoach={loggedCoach} isCommunity={isCommunity} onViewProfile={viewProfile} />}
      ...
      {tab === "profile" && <ProfileScreen roster={roster} assessments={safeAssessments} setAssessments={setAssessments} config={safeConfig} setConfig={setConfig} selectedKidId={selectedKidId} setSelectedKidId={setSelectedKidId} onEditAssessment={editAssessment} isCommunity={isCommunity} attendance={attendance || []} selections={selections} />}
      ...
      <div style={{
        ...
        {[...NAV_ITEMS.filter(n => !(isCommunity && n.key === "settings") && !(!isCommunity && !isAdmin && n.key === "attendance")), ...(isAdmin ? [{ key: "admin", icon: "👑", label: "Admin" }] : [])].map(n => (
