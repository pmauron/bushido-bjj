  ...
  const loadDemoData = () => {
    ...
    for(let d=new Date(sD);d<=eD;d=ad(d,1)){
      ...
      if([1,3,5].includes(dow)){
        ...
        newAtt.push({key:`${dStr}|10:00|group1|${DEMO_GYM}`,date:dStr,time:"10:00",classTypeId:"group1",scheduleId:null,gym:DEMO_GYM,coach:DEMO_COACH,durationMin:60,records:rec});
      }
      if([3,6].includes(dow)){
        const rec={}; cKids.forEach(k=>{const r=Math.random();if(r<0.75)rec[k.id]="attend";else if(r<0.9)rec[k.id]="excused";});
        newAtt.push({key:`${dStr}|12:00|comp|${DEMO_GYM}`,date:dStr,time:"12:00",classTypeId:"comp",scheduleId:null,gym:DEMO_GYM,coach:DEMO_COACH,durationMin:90,records:rec});
      }
    }
    // Goals & Promotion Log
    const newGoals = {};
    ...
    setConfig(prev => ({ ...prev, goals: newGoals, promotionLog: newPLog }));
  ...
  return (
    <div style={s.page}>
      ...
      <div style={{ display: "flex", marginBottom: 12, borderRadius: 8, overflow: "hidden", border: `1px solid ${C.border}` }}>
        {[{ key: "record", label: "📋 Record" }, { key: "history", label: "📖 History" }, { key: "analytics", label: "📊 Analytics" }].map(t => (
