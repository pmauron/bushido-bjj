...
function ProfileScreen({ roster, assessments, setAssessments, config, setConfig, selectedKidId, setSelectedKidId, onEditAssessment, isCommunity, attendance }) {
        ...
        <div style={{ ...s.card, background: `linear-gradient(135deg, ${C.card} 0%, ${C.red}11 100%)` }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
            ...
            {(() => {
              ...
              const days = (attendance || []).filter(r => r.date >= cutoff);
              if (!days.length) return null;
              const present = days.filter(r => r.present.includes(kid.id)).length;
              const rate = ((present / days.length) * 100).toFixed(0);
              const rateColor = rate >= 75 ? C.green : rate >= 50 ? C.orange : C.red;
              return <span style={s.badge(rateColor)}>📋 {rate}% ({present}/{days.length})</span>;
            })()}
          </div>
        </div>
      )}
...
/* ━━━ LOGIN SCREEN ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
/* ━━━ ATTENDANCE SCREEN ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
function AttendanceScreen({ roster, attendance, setAttendance, config, loggedGym, isAdmin }) {
  const [date, setDate] = useState(today());
  const [filterGym, setFilterGym] = useState(loggedGym || "");

  const activeKids = roster.filter(k => k.active).sort((a, b) => a.name.localeCompare(b.name));
  const filteredKids = filterGym ? activeKids.filter(k => kidInGym(k, filterGym)) : activeKids;

  // Get attendance for selected date
  const dayKey = date;
  const dayRecord = attendance.find(r => r.date === dayKey) || { date: dayKey, present: [] };

  const isPresent = (kidId) => dayRecord.present.includes(kidId);

  const togglePresence = (kidId) => {
    setAttendance(prev => {
      const arr = Array.isArray(prev) ? prev : [];
      const existing = arr.findIndex(r => r.date === dayKey);
      const present = existing >= 0 ? [...arr[existing].present] : [];
      const idx = present.indexOf(kidId);
      if (idx >= 0) present.splice(idx, 1);
      else present.push(kidId);
      const newRecord = { date: dayKey, present };
      if (existing >= 0) { const next = [...arr]; next[existing] = newRecord; return next; }
      return [...arr, newRecord];
    });
  };

  const toggleAll = () => {
    const allIds = filteredKids.map(k => k.id);
    const allPresent = allIds.every(id => isPresent(id));
    setAttendance(prev => {
      const arr = Array.isArray(prev) ? prev : [];
      const existing = arr.findIndex(r => r.date === dayKey);
      const currentPresent = existing >= 0 ? [...arr[existing].present] : [];
      let present;
      if (allPresent) {
        present = currentPresent.filter(id => !allIds.includes(id));
      } else {
        present = [...new Set([...currentPresent, ...allIds])];
      }
      const newRecord = { date: dayKey, present };
      if (existing >= 0) { const next = [...arr]; next[existing] = newRecord; return next; }
      return [...arr, newRecord];
  ...
  const presentCount = filteredKids.filter(k => isPresent(k.id)).length;

  // Stats: attendance rate per kid (last 30 days)
  const [showStats, setShowStats] = useState(false);
  const last30 = useMemo(() => {
    const d = new Date(); d.setDate(d.getDate() - 30);
    const cutoff = d.toISOString().slice(0, 10);
    return (attendance || []).filter(r => r.date >= cutoff);
  }, [attendance]);

  const kidStats = useMemo(() => {
    const classDays = last30.length;
    if (!classDays) return {};
    const stats = {};
    filteredKids.forEach(k => {
      const present = last30.filter(r => r.present.includes(k.id)).length;
      stats[k.id] = { present, total: classDays, rate: ((present / classDays) * 100).toFixed(0) };
    });
    return stats;
  }, [last30, filteredKids]);
  ...
  return (
    <div style={s.page}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        <button style={s.btnSm} onClick={() => setShowStats(!showStats)}>{showStats ? "📋 Check-in" : "📊 Stats"}</button>
      </div>
        ...
        <>
          <div style={{ ...s.card, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>
              ✅ {presentCount}/{filteredKids.length} present
            </span>
            <button style={s.btnSm} onClick={toggleAll}>
              {presentCount === filteredKids.length ? "Uncheck All" : "Check All"}
            </button>
          </div>

          <div style={{ border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden" }}>
            {filteredKids.map((k, i) => (
              <button key={k.id} onClick={() => togglePresence(k.id)} style={{
                display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", width: "100%",
                background: isPresent(k.id) ? C.green + "11" : "transparent",
                border: "none", borderBottom: i < filteredKids.length - 1 ? `1px solid ${C.border}` : "none",
                cursor: "pointer", textAlign: "left", color: C.text, transition: "background 0.1s",
              }}>
                <div style={{
                  width: 28, height: 28, borderRadius: 14, border: isPresent(k.id) ? `2px solid ${C.green}` : `2px solid ${C.border}`,
                  background: isPresent(k.id) ? C.green : "transparent",
            ...
            {filteredKids.length === 0 && <div style={{ padding: 16, textAlign: "center", color: C.textDim, fontSize: 13 }}>No active kids {filterGym ? `in ${filterGym}` : ""}</div>}
          </div>
        </>
      )}
      ...
      {showStats && (
        <>
          <div style={{ ...s.card, marginBottom: 10 }}>
            <div style={{ fontSize: 12, color: C.textDim }}>Last 30 days · {last30.length} class days recorded</div>
          </div>
          <div style={{ border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden" }}>
            {filteredKids.sort((a, b) => (kidStats[b.id]?.rate || 0) - (kidStats[a.id]?.rate || 0)).map((k, i) => {
              const st = kidStats[k.id] || { present: 0, total: 0, rate: 0 };
              const rateNum = parseInt(st.rate);
              const rateColor = rateNum >= 75 ? C.green : rateNum >= 50 ? C.orange : C.red;
              return (
                <div key={k.id} style={{
                  display: "flex", alignItems: "center", gap: 10, padding: "10px 14px",
                  borderBottom: i < filteredKids.length - 1 ? `1px solid ${C.border}` : "none",
                }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, fontSize: 13, color: C.text }}>{k.name}</div>
                    <div style={{ fontSize: 11, color: C.textDim }}>{st.present}/{st.total} classes</div>
                  </div>
                  <div style={{ fontWeight: 800, fontSize: 16, color: rateColor, minWidth: 44, textAlign: "right" }}>{st.rate}%</div>
                </div>
              );
            })}
          </div>
...
export default function App() {
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", WebkitFontSmoothing: "antialiased" }}>
      ...
      {tab === "attendance" && <AttendanceScreen roster={roster} attendance={attendance || []} setAttendance={setAttendance} config={safeConfig} loggedGym={loggedGym} isAdmin={isAdmin} />}
      ...
      {tab === "profile" && <ProfileScreen roster={roster} assessments={safeAssessments} setAssessments={setAssessments} config={safeConfig} setConfig={setConfig} selectedKidId={selectedKidId} setSelectedKidId={setSelectedKidId} onEditAssessment={editAssessment} isCommunity={isCommunity} attendance={attendance || []} />}
