...
function HomeScreen({ roster, attendance, assessments, config, selections, loggedCoach, loggedGym, isAdmin, isCommunity, onNavigate }) {
  ...
  const gymKids = activeKids.filter(k => !loggedGym || isAdmin || kidInGym(k, loggedGym));
  ...
  // Classes: today's scheduled classes not yet recorded
  const todayStr = today();
  const todayDow = nowDate.getDay();
  const gymFilter = isAdmin ? null : loggedGym;
  const todaySlots = schedule.filter(sl => sl.day === todayDow && (!gymFilter || sl.gym === gymFilter));
  const todayRecords = (attendance || []).filter(r => r.date === todayStr);
  const unrecordedClasses = todaySlots.filter(slot => {
    const recKey = `${todayStr}|${slot.time}|${slot.classTypeId}|${slot.gym}`;
  ...
  // Quick pulse metrics
  const avgWeekly = gymKids.length ? gymKids.reduce((s2, k) => {
    const recs = gymFilter ? att90.filter(r => r.gym === gymFilter) : att90;
    return s2 + recs.filter(r => r.records?.[k.id] === "attend").length / weeks90;
  }, 0) / gymKids.length : 0;
  const allScores = cycleAss.filter(a => gymKids.some(k => k.id === a.kidId)).map(a => computeSubtotals(a.scores, config).final);
  const avgScore = allScores.length ? allScores.reduce((s2, v) => s2 + v, 0) / allScores.length : 0;
  ...
  // ── Tiles ──
  const tiles = [
    { key: "classes", icon: "\ud83d\udccb", label: "Classes", desc: "Record today\u2019s attendance", badge: unrecordedClasses || null, badgeColor: C.orange, badgeLabel: "today" },
    ...
    { key: "rankings", icon: "\ud83c\udfc6", label: "Rankings", desc: "Leaderboards & teams", badge: null, hidden: false },
  ...
  return (
    <div style={s.page}>
      ...
      {asc && (() => {
        ...
        return (
          <div style={{ padding: "10px 14px", borderRadius: 10, marginBottom: 4, marginTop: 12, fontSize: 12, fontWeight: 600,
            display: "flex", alignItems: "center", justifyContent: "space-between",
            background: isEarly ? "#4CAF5012" : "#FF980012",
            border: `1px solid ${isEarly ? "#4CAF5033" : "#FF980033"}`,
            color: isEarly ? "#4CAF50" : "#FF9800" }}>
            <span>{isEarly ? `${asc.cycle} scoring open` : `${asc.cycle} assessment due`} \u2014 {overdueCount} left</span>
            <button onClick={() => onNavigate("score")} style={{
              marginLeft: 12, padding: "4px 12px", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer",
      ...
      <div style={{ marginTop: 24 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8 }}>
          {[
            { label: "Active", value: gymKids.length, color: C.text },
            { label: "Avg/wk", value: avgWeekly.toFixed(1), color: avgWeekly >= 3 ? C.green : avgWeekly >= 2 ? C.orange : C.red },
            { label: "Avg Score", value: avgScore ? fmt(avgScore) : "\u2014", color: avgScore >= 4 ? C.green : avgScore >= 3 ? C.orange : (avgScore > 0 ? C.red : C.textDim) },
            { label: "At Risk", value: atRiskCount, color: atRiskCount === 0 ? C.green : "#e74c3c" },
          ].map((m, i) => (
            <div key={i} style={{ background: C.card, borderRadius: 12, padding: "12px 6px", textAlign: "center", border: `1px solid ${C.border}` }}>
              <div style={{ fontSize: 20, fontWeight: 900, color: m.color, fontFamily: "'Bebas Neue', sans-serif", lineHeight: 1 }}>{m.value}</div>
              <div style={{ fontSize: 8, color: C.textDim, textTransform: "uppercase", letterSpacing: 0.5, marginTop: 4 }}>{m.label}</div>
            </div>
          ))}
        </div>
      </div>
