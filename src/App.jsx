...
const PAGE_HELP = {
  ...
  reports: {
    en: "Academy dashboard: roster health (belt, age, weight distribution by gym), attendance insights (top attenders, at-risk kids), class fill rates, assessment coverage with score distribution, overdue list, and competition team overview. Admin sees all gyms or filters; coaches/community see their gym only.",
    zh: "学院仪表盘：学员健康（腰带、年龄、体重分布），出勤洞察（最稳定学员、需关注学员），课程出勤率，评估覆盖与成绩分布，逾期名单，竞赛队概览。管理员可查看全部或筛选，教练/社区仅显示本道馆。",
  },
  profile: {
...
function HomeScreen({ roster, attendance, assessments, config, selections, loggedCoach, loggedGym, isAdmin, isCommunity, onNavigate }) {
  const [gymFilter, setGymFilter] = useState(isAdmin ? "" : (loggedGym || ""));
  ...
  return (
    <div style={s.page}>
      ...
      <div style={{ fontSize: 12, color: "#888", marginBottom: 16 }}>
        ...
        {!isAdmin && loggedGym && <span style={{ marginLeft: 8, padding: "2px 8px", borderRadius: 5, fontSize: 10, fontWeight: 700, background: "#C41E3A" + "20", color: "#C41E3A" }}>{"\ud83d\udccd"} {loggedGym}</span>}
      </div>
      {isAdmin && (
        <div style={{ marginBottom: 12 }}>
          <select value={gymFilter} onChange={e => setGymFilter(e.target.value)} style={{ background: "#1c1c1c", border: `1px solid ${"#2a2a2a"}`, borderRadius: 6, color: "#e8e8e8", padding: "6px 10px", fontSize: 12 }}>
      ...
      {(() => {
        ...
        if (!asc) return null;
        const isEarly = asc.phase === "early";
        return (
          <div style={{ padding: "10px 14px", borderRadius: 8, marginBottom: 12, fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", justifyContent: "space-between",
            background: isEarly ? "#4CAF5018" : "#FF980018",
            border: `1px solid ${isEarly ? "#4CAF5033" : "#FF980033"}`,
            color: isEarly ? "#4CAF50" : "#FF9800" }}>
            {isEarly
              ? `${asc.cycle} assessment has started \u2014 you can begin scoring kids`
              : `${asc.cycle} assessment is due \u2014 make sure to assess your kids`}
            <button onClick={() => onNavigate("score")} style={{
              marginLeft: 12, padding: "4px 14px", borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: "pointer",
              background: isEarly ? "#4CAF5022" : "#FF980022",
              border: `1px solid ${isEarly ? "#4CAF5055" : "#FF980055"}`,
              color: isEarly ? "#4CAF50" : "#FF9800", whiteSpace: "nowrap", flexShrink: 0,
            }}>Go to Score \u203a</button>
          </div>
        );
      })()}
      ...
      {yesterdayCards.length === 0 ? (
        <div style={{ ...s.card, textAlign: "center", color: "#888", fontSize: 13 }}>No classes scheduled yesterday</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 6 }}>
          {yesterdayCards.map((c, i) => (
            <div key={i} style={{ ...s.card, marginBottom: 0, padding: "12px 14px", display: "flex", alignItems: "center", gap: 12, borderLeft: `3px solid ${c.ct ? c.ct.color : "#888"}` }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  <span style={{ fontSize: 14, fontWeight: 700 }}>{c.ct ? c.ct.name : c.slot.classTypeId}</span>
                  <span style={{ fontSize: 11, color: "#888" }}>{c.slot.time}</span>
                  {isAdmin && <span style={{ fontSize: 10, color: "#555" }}>{"\u00b7"} {c.slot.gym}</span>}
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ height: 4, background: "#1c1c1c", borderRadius: 2, overflow: "hidden", flex: 1 }}>
                    <div style={{ height: "100%", width: `${Math.min(100, (c.attendCount / c.capacity) * 100)}%`, background: c.ct ? c.ct.color : "#888", borderRadius: 2 }} />
                  </div>
                  <span style={{ fontSize: 11, color: c.attendCount > 0 ? (c.ct ? c.ct.color : "#e8e8e8") : "#888", fontWeight: 600, flexShrink: 0 }}>{c.attendCount}/{c.capacity}</span>
                </div>
              </div>
            </div>
          ))}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 6 }}>
        {[
          ...
          { label: "Active Last 30d", value: activePct30 + "%", sub: `${kidsAttended30.size} of ${activeKids.length} trained`, color: activePct30 >= 80 ? "#4CAF50" : activePct30 >= 50 ? "#FF9800" : "#C41E3A" },
          { label: "Avg Class Attendance", value: avgClassAtt30, sub: "last 30 days" },
          { label: "Top Class (30d)", value: topClassDisplay, sub: topClassSub, color: topClass ? "#4CAF50" : undefined, small: true },
        ].map((card, i) => (
          <div key={i} style={{ ...s.card, textAlign: "center", padding: "12px 10px", marginBottom: 0 }}>
            <div style={{ fontSize: 10, color: "#888", textTransform: "uppercase", letterSpacing: 1, marginBottom: 4 }}>{card.label}</div>
            <div style={{ fontSize: card.small ? 18 : 28, fontWeight: 900, color: card.color || "#e8e8e8", fontFamily: "'Bebas Neue', sans-serif", lineHeight: 1 }}>{card.value}</div>
            {card.sub && <div style={{ fontSize: 10, color: "#C41E3A", fontWeight: 600, marginTop: 2 }}>{card.sub}</div>}
          </div>
        ))}
      </div>
      ...
      {!isCommunity && <>
        ...
        {overdueKids.length > 0 && (
          <div style={{ ...s.card, borderLeft: `3px solid ${"#FF9800"}` }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
              ...
              <div style={{ background: "#FF9800" + "20", color: "#FF9800", fontSize: 11, fontWeight: 800, padding: "2px 8px", borderRadius: 10 }}>{overdueKids.length}</div>
            </div>
            <div style={{ fontSize: 12, color: "#888", marginBottom: 8 }}>{overdueKids.length} kids not yet scored {"\u00b7"} {currentCycle}</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
              {overdueKids.slice(0, 4).map(k => (
                <span key={k.id} style={{ fontSize: 11, padding: "3px 8px", borderRadius: 6, background: "#FF9800" + "11", border: `1px solid ${"#FF9800"}33`, color: "#e8e8e8" }}>{k.name}</span>
              ))}
              {overdueKids.length > 4 && <span style={{ fontSize: 11, padding: "3px 8px", color: "#FF9800", fontWeight: 600 }}>+{overdueKids.length - 4} more</span>}
            </div>
            <CTABtn label="Go to Score" color={"#FF9800"} onClick={() => onNavigate("score")} />
          </div>
        )}
        {promoReady.length > 0 && (
          <div style={{ ...s.card, borderLeft: `3px solid ${"#4CAF50"}` }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
              ...
              <div style={{ background: "#4CAF50" + "20", color: "#4CAF50", fontSize: 11, fontWeight: 800, padding: "2px 8px", borderRadius: 10 }}>{promoReady.length}</div>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {promoReady.slice(0, 5).map(p => (
                <div key={p.kid.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
                  ...
                  <span style={{ color: "#888", fontSize: 11 }}>{p.kid.belt} {"\u00b7"} {p.beltReady ? "belt ready" : `${p.kid.stripes || 0} \u2192 ${(p.kid.stripes || 0) + 1} stripes`}</span>
                </div>
              ))}
              {promoReady.length > 5 && <div style={{ fontSize: 11, color: "#4CAF50", fontWeight: 600 }}>+{promoReady.length - 5} more</div>}
            </div>
            <CTABtn label="Go to Promotions" color={"#4CAF50"} onClick={() => onNavigate("promotion")} />
          </div>
        )}
        {atRisk.length > 0 && atRisk[0].classes30d < 3 && (
          <div style={{ ...s.card, borderLeft: `3px solid ${"#C41E3A"}` }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
              ...
              <div style={{ background: "#C41E3A20", color: "#C41E3A", fontSize: 11, fontWeight: 800, padding: "2px 8px", borderRadius: 10 }}>{atRisk.filter(a => a.classes30d < 3).length}</div>
            </div>
            <div style={{ fontSize: 12, color: "#888", marginBottom: 8 }}>Lowest training frequency in last 30 days</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
              {atRisk.map((a, i) => (
                <div key={a.kid.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: 12, padding: "6px 0", borderBottom: i < atRisk.length - 1 ? `1px solid ${"#2a2a2a"}` : "none" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <span style={{ fontSize: 10, color: "#555", fontWeight: 600, width: 16 }}>{i + 1}.</span>
                    <span>{a.kid.name}</span>
                  </div>
                  <span style={{ color: a.classes30d === 0 ? "#C41E3A" : "#FF9800", fontSize: 11, fontWeight: 600 }}>
                    {a.classes30d === 0 ? "0 classes" : `${a.classes30d} class${a.classes30d > 1 ? "es" : ""}`}
                  </span>
                </div>
              ))}
            </div>
            <CTABtn label="View Roster by Training" color={"#C41E3A"} onClick={() => onNavigate("roster_training")} />
          </div>
        )}
        {overdueKids.length === 0 && promoReady.length === 0 && (atRisk.length === 0 || atRisk[0].classes30d >= 3) && (
          <div style={{ ...s.card, textAlign: "center", color: "#4CAF50", fontSize: 13 }}>{"\u2713"} No action items {"\u2014"} looking good!</div>
        )}
      </>}

      <h2 style={s.h2}>Recent Activity</h2>
      {recentFive.length === 0 ? (
        <div style={{ ...s.card, textAlign: "center", color: "#888", fontSize: 13 }}>No recent activity</div>
      ) : (
        <div style={s.card}>
          {recentFive.map((item, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < recentFive.length - 1 ? `1px solid ${"#2a2a2a"}` : "none" }}>
              <span style={{ fontSize: 14, flexShrink: 0 }}>{item.icon}</span>
              <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 12, color: "#e8e8e8", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.text}</div></div>
              <span style={{ fontSize: 10, color: "#888", flexShrink: 0 }}>{daysSince(item.date)}</span>
            </div>
          ))}
        </div>
      )}
        ...
        <div style={{ ...s.card, background: `linear-gradient(135deg, ${C.card} 0%, ${C.red}11 100%)` }}>
          <div style={{ display: "flex", gap: 14, alignItems: "center" }}>
            <div style={{ position: "relative", flexShrink: 0 }}>
              ...
              {!isCommunity && <button onClick={() => setModal(kid)} style={{ position: "absolute", bottom: -4, right: -4, width: 24, height: 24, borderRadius: "50%", background: C.red, border: `2px solid ${C.card}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 11, padding: 0 }}>📷</button>}
            </div>
            <div>
...
function ReportingScreen({ roster, assessments, config, onViewProfile, onScore, readOnly, isAdmin, loggedGym, selections, attendance }) {
  ...
  const kidWeeklyAvg = (kidId) => {
    const ct = att90.filter(r => r.records?.[kidId] === "attend").length;
  ...
  const compTeamCount = roster.filter(k => k.active && compTeamIds.has(k.id) && (!filterGym || kidInGym(k, filterGym))).length;

  const academyAvgWeekly = activeKids.length ? activeKids.reduce((s, k) => s + kidWeeklyAvg(k.id), 0) / activeKids.length : 0;
  ...
  const MiniBar = ({ value, max, color, width = 80, height = 6 }) => {
    const pct = max > 0 ? Math.min(100, (value / max) * 100) : 0;
    return (
      <div style={{ width, height, background: C.card2, borderRadius: height / 2, overflow: "hidden" }}>
  ...
  activeKids.forEach(k => { if (beltCounts[k.belt] !== undefined) beltCounts[k.belt]++; else beltCounts[k.belt] = 1; });
  const totalBelts = activeKids.length;
  ...
  const gymStats = gymList.map(gym => {
    ...
    const gInactive = inactiveKids.filter(k => kidInGym(k, gym));
    const gAssessed = gKids.filter(k => assessedIds.has(k.id)).length;
    const gAvgWeekly = gKids.length ? gKids.reduce((s, k) => s + kidWeeklyAvg(k.id), 0) / gKids.length : 0;
    const gScores = uniqueAssessed.filter(a => { const k = roster.find(x => x.id === a.kidId); return k && kidInGym(k, gym); }).map(a => computeSubtotals(a.scores, config).final);
    const gAvgScore = gScores.length ? gScores.reduce((s, v) => s + v, 0) / gScores.length : 0;
    return { gym, active: gKids.length, inactive: gInactive.length, assessed: gAssessed, avgWeekly: gAvgWeekly, avgScore: gAvgScore };
  });

  // ── Attendance insights ──
  const kidAttList = activeKids.map(k => ({ kid: k, avg: kidWeeklyAvg(k.id) }));
  const topAttenders = kidAttList.filter(k => k.avg > 0).sort((a, b) => b.avg - a.avg).slice(0, 5);
  const atRiskKids = kidAttList.filter(k => k.avg < 1).sort((a, b) => a.avg - b.avg);
  ...
  allScores.forEach(sc => { const b = buckets.find(b => sc >= b.min && sc < b.max); if (b) b.count++; });
    ...
    <div style={s.page}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
        ...
        <div style={{ ...s.card, padding: 16, background: `linear-gradient(135deg, ${C.card} 60%, ${C.red}08 100%)` }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <span style={{ fontSize: 32, fontWeight: 900, color: C.text, fontFamily: "'Bebas Neue', sans-serif", lineHeight: 1 }}>{activeKids.length}</span>
              <span style={{ fontSize: 12, color: C.textDim, marginLeft: 6 }}>kids</span>
            </div>
            {inactiveKids.length > 0 && <div style={{ fontSize: 10, color: C.textMuted, background: C.card2, padding: "2px 8px", borderRadius: 10 }}>{inactiveKids.length} inactive</div>}
          </div>
        </div>
        ...
        <div style={{ ...s.card, padding: 16, background: `linear-gradient(135deg, ${C.card} 60%, ${C.green}08 100%)` }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
            <span style={{ fontSize: 32, fontWeight: 900, color: academyAvgWeekly >= 3 ? C.green : academyAvgWeekly >= 2 ? C.orange : C.red, fontFamily: "'Bebas Neue', sans-serif", lineHeight: 1 }}>{academyAvgWeekly.toFixed(1)}</span>
            <span style={{ fontSize: 12, color: C.textDim }}>cls/wk</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 0 }}>
        ...
        <div style={{ ...s.card, padding: 16, display: "flex", alignItems: "center", gap: 14 }}>
          <Ring pct={activeKids.length ? (assessedCount / activeKids.length * 100) : 0} size={56} stroke={5} color={assessedCount === activeKids.length ? C.green : C.red}>
            {activeKids.length ? Math.round(assessedCount / activeKids.length * 100) : 0}%
          </Ring>
          <div>
            <div style={{ fontSize: 9, color: C.textDim, textTransform: "uppercase", letterSpacing: 1.2 }}>Assessed</div>
            <div style={{ fontSize: 16, fontWeight: 800, color: C.text }}>{assessedCount}/{activeKids.length}</div>
        ...
        {/* Competition Team */}
        <div style={{ ...s.card, padding: 16, display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ width: 56, height: 56, borderRadius: 28, background: `${C.orange}18`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, flexShrink: 0 }}>🥋</div>
          <div>
            <div style={{ fontSize: 9, color: C.textDim, textTransform: "uppercase", letterSpacing: 1.2 }}>Comp Team</div>
            <div style={{ fontSize: 16, fontWeight: 800, color: C.text }}>{compTeamCount}</div>
            <div style={{ fontSize: 10, color: C.textDim }}>selected</div>
          </div>
        </div>
      </div>
      ...
      {/* Belt Distribution — stacked horizontal bar */}
      <div style={s.card}>
        ...
        {totalBelts > 0 ? (
          ...
          <div style={{ color: C.textDim, fontSize: 12, textAlign: "center" }}>No active kids</div>
        )}
      </div>

      {/* Age & Weight Distribution — side by side */}
      {activeKids.length > 0 && (() => {
        const AGE_CATS = ["U8","U10","U12","U14"];
        const AGE_COLORS = { U8: "#9C27B0", U10: "#2196F3", U12: "#FF9800", U14: "#E53935" };
        const ageDist = {};
        AGE_CATS.forEach(ac => ageDist[ac] = 0);
        activeKids.forEach(k => { const ac = ageCat(ageAt(k.dob, today())); ageDist[ac] = (ageDist[ac] || 0) + 1; });
        ...
        activeKids.forEach(k => { const wr = WEIGHT_RANGES.find(r => k.weight >= r.min && k.weight < r.max); if (wr) wr.count++; });
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            ...
            <div style={s.card}>
              ...
              <div style={{ display: "flex", gap: 6, alignItems: "flex-end", height: 70, marginBottom: 8 }}>
                {AGE_CATS.map(ac => {
                  const h = maxAge > 0 ? (ageDist[ac] / maxAge) * 60 : 0;
                  return (
                    <div key={ac} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
                      ...
                      <div style={{ width: "100%", height: Math.max(h, 2), background: AGE_COLORS[ac], borderRadius: 4, opacity: ageDist[ac] > 0 ? 0.85 : 0.15, transition: "height 0.4s" }} />
                    </div>
                  );
                })}
              </div>
              <div style={{ display: "flex", gap: 6 }}>
                {AGE_CATS.map(ac => (
                  <div key={ac} style={{ flex: 1, textAlign: "center", fontSize: 10, color: C.textDim, fontWeight: 600 }}>{ac}</div>
                ))}
              </div>
            </div>
            ...
            <div style={s.card}>
              ...
              <div style={{ display: "flex", gap: 4, alignItems: "flex-end", height: 70, marginBottom: 8 }}>
                {WEIGHT_RANGES.map(wr => {
                  const h = maxW > 0 ? (wr.count / maxW) * 60 : 0;
                  return (
                    <div key={wr.label} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
                      ...
                      <div style={{ width: "100%", height: Math.max(h, 2), background: wr.color, borderRadius: 4, opacity: wr.count > 0 ? 0.85 : 0.15, transition: "height 0.4s" }} />
                    </div>
                  );
                })}
              </div>
              <div style={{ display: "flex", gap: 4 }}>
                {WEIGHT_RANGES.map(wr => (
                  <div key={wr.label} style={{ flex: 1, textAlign: "center", fontSize: 9, color: C.textDim }}>{wr.label}</div>
                ))}
              </div>
            </div>
          </div>
      ...
      {gymStats.length > 1 && (
        <div style={s.card}>
          <div style={{ fontSize: 10, color: C.textDim, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>By Gym</div>
          {gymStats.map(g => (
            <div key={g.gym} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: `1px solid ${C.border}22` }}>
              <div style={{ width: 80, fontWeight: 700, fontSize: 13, color: C.text }}>{g.gym}</div>
              <div style={{ flex: 1, display: "flex", gap: 16, alignItems: "center" }}>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 18, fontWeight: 900, color: C.text, fontFamily: "'Bebas Neue', sans-serif" }}>{g.active}</div>
                  <div style={{ fontSize: 8, color: C.textDim, textTransform: "uppercase" }}>Active</div>
                </div>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 18, fontWeight: 900, color: g.avgWeekly >= 3 ? C.green : g.avgWeekly >= 2 ? C.orange : C.red, fontFamily: "'Bebas Neue', sans-serif" }}>{g.avgWeekly.toFixed(1)}</div>
                  <div style={{ fontSize: 8, color: C.textDim, textTransform: "uppercase" }}>Cls/wk</div>
                </div>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 18, fontWeight: 900, color: g.avgScore >= 4 ? C.green : g.avgScore >= 3 ? C.orange : (g.avgScore > 0 ? C.red : C.textDim), fontFamily: "'Bebas Neue', sans-serif" }}>{g.avgScore ? fmt(g.avgScore) : "—"}</div>
                  <div style={{ fontSize: 8, color: C.textDim, textTransform: "uppercase" }}>Avg Score</div>
                </div>
                <div style={{ textAlign: "center" }}>
                  <Ring pct={g.active ? (g.assessed / g.active * 100) : 0} size={38} stroke={3} color={g.assessed === g.active ? C.green : C.red}>
                    {g.active ? Math.round(g.assessed / g.active * 100) : 0}%
                  </Ring>
                  <div style={{ fontSize: 8, color: C.textDim, textTransform: "uppercase", marginTop: 2 }}>Scored</div>
                </div>
              </div>
            </div>
          ))}
      ...
      {/* ═══════════ ATTENDANCE ═══════════ */}
      <SectionHead icon="📈" title="Attendance (90 days)" />

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {/* Top Attenders */}
        <div style={s.card}>
          <div style={{ fontSize: 10, color: C.textDim, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>🏅 Most Consistent</div>
          {topAttenders.length === 0 ? (
            <div style={{ fontSize: 12, color: C.textDim, textAlign: "center", padding: 12 }}>No attendance data</div>
          ) : topAttenders.map((k, i) => (
            <div key={k.kid.id} onClick={() => onViewProfile(k.kid.id)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", cursor: "pointer", borderBottom: i < topAttenders.length - 1 ? `1px solid ${C.border}22` : "none" }}>
              <div style={{ width: 20, height: 20, borderRadius: 10, background: i === 0 ? C.red : C.card2, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 900, color: i === 0 ? "#fff" : C.textDim, flexShrink: 0 }}>{i + 1}</div>
              <div style={{ flex: 1, fontSize: 12, fontWeight: 600, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{k.kid.name}</div>
              <div style={{ fontSize: 13, fontWeight: 800, color: C.green }}>{k.avg}</div>
            </div>
          ))}
        </div>
        ...
        {/* At Risk */}
        <div style={s.card}>
          <div style={{ fontSize: 10, color: C.textDim, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>⚠️ At Risk (&lt;1/wk)</div>
          {atRiskKids.length === 0 ? (
            <div style={{ fontSize: 12, color: C.green, textAlign: "center", padding: 12 }}>✓ Everyone training</div>
          ) : (
            <>
              {atRiskKids.slice(0, 5).map((k, i) => (
                <div key={k.kid.id} onClick={() => onViewProfile(k.kid.id)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 0", cursor: "pointer", borderBottom: i < Math.min(4, atRiskKids.length - 1) ? `1px solid ${C.border}22` : "none" }}>
                  <div style={{ flex: 1, fontSize: 12, fontWeight: 600, color: C.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{k.kid.name}</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: k.avg === 0 ? "#e74c3c" : C.orange }}>{k.avg === 0 ? "0" : k.avg}</div>
                </div>
              ))}
              {atRiskKids.length > 5 && <div style={{ fontSize: 10, color: C.textDim, textAlign: "center", marginTop: 6 }}>+{atRiskKids.length - 5} more</div>}
            </>
          )}
        </div>
      </div>
      ...
      {(() => {
        const classTypes = config.classTypes || [];
        // Group attendance records by classTypeId
        const classStats = {};
        att90.forEach(r => {
          const ctId = r.classTypeId || "_unknown";
          if (!classStats[ctId]) classStats[ctId] = { sessions: 0, totalAttend: 0, totalRegistered: 0 };
          classStats[ctId].sessions++;
          const recs = r.records || {};
          const attendCount = Object.values(recs).filter(v => v === "attend").length;
          const registeredCount = Object.keys(recs).length;
          classStats[ctId].totalAttend += attendCount;
          classStats[ctId].totalRegistered += registeredCount;
        });
        // Filter by gym if needed
        const gymAtt90 = filterGym ? att90.filter(r => r.gym === filterGym) : att90;
        const gymClassStats = {};
        gymAtt90.forEach(r => {
          const ctId = r.classTypeId || "_unknown";
          if (!gymClassStats[ctId]) gymClassStats[ctId] = { sessions: 0, totalAttend: 0 };
          gymClassStats[ctId].sessions++;
          const attendCount = Object.values(r.records || {}).filter(v => v === "attend").length;
          gymClassStats[ctId].totalAttend += attendCount;
        });

        const classRows = Object.entries(gymClassStats).map(([ctId, st]) => {
          const ct = classTypes.find(c => c.id === ctId);
          const name = ct?.name || (ctId === "_unknown" ? "Untyped" : ctId);
          const cap = ct?.capacity || 25;
          const avgAtt = st.sessions > 0 ? st.totalAttend / st.sessions : 0;
          const fillPct = cap > 0 ? (avgAtt / cap) * 100 : 0;
          const color = ct?.color || (ct?.category === "competition" ? "#FF9800" : "#4CAF50");
          return { ctId, name, sessions: st.sessions, avgAtt, cap, fillPct, color, category: ct?.category || "group" };
        }).sort((a, b) => b.sessions - a.sessions);

        const maxSessions = Math.max(1, ...classRows.map(r => r.sessions));

        return classRows.length === 0 ? (
          <div style={{ ...s.card, textAlign: "center", color: C.textDim, fontSize: 12 }}>No class data in the last 90 days</div>
        ) : (
          <div style={s.card}>
            {classRows.map((cr, i) => (
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 10, color: C.textDim }}>{cr.sessions} sessions · {cr.category}</div>
                    </div>
                  </div>
                  <div style={{ textAlign: "right" }}>
      ...
      {allScores.length > 0 && (
        <div style={s.card}>
          <div style={{ fontSize: 10, color: C.textDim, textTransform: "uppercase", letterSpacing: 1, marginBottom: 10 }}>Score Distribution</div>
          <div style={{ display: "flex", gap: 6, alignItems: "flex-end", height: 80, marginBottom: 8 }}>
            {buckets.map(b => {
              const h = maxBucket > 0 ? (b.count / maxBucket) * 70 : 0;
              return (
                <div key={b.label} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center" }}>
                  ...
                  <div style={{ width: "100%", height: Math.max(h, 2), background: b.color, borderRadius: 4, opacity: b.count > 0 ? 0.85 : 0.2, transition: "height 0.4s" }} />
                </div>
              );
            })}
          </div>
          <div style={{ display: "flex", gap: 6 }}>
            {buckets.map(b => (
              <div key={b.label} style={{ flex: 1, textAlign: "center", fontSize: 9, color: C.textDim }}>{b.label}</div>
            ))}
          </div>
          {avgScore > 0 && (
            <div style={{ textAlign: "center", marginTop: 10, fontSize: 11, color: C.textDim }}>
              Academy avg: <span style={{ fontWeight: 800, color: avgScore >= 4 ? C.green : avgScore >= 3 ? C.orange : C.red, fontSize: 14 }}>{fmt(avgScore)}</span> / 5
            </div>
          )}
        </div>
      )}
      ...
      {/* Overdue */}
      <div style={{ ...s.card, borderLeft: overdueKids.length > 0 ? `3px solid ${C.red}` : `3px solid ${C.green}` }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: overdueKids.length > 0 ? 10 : 0 }}>
          <div style={{ fontSize: 10, color: C.textDim, textTransform: "uppercase", letterSpacing: 1 }}>
            {overdueKids.length > 0 ? `⏰ Not Yet Assessed (${overdueKids.length})` : "✓ All Assessed"}
          </div>
          {overdueKids.length > 0 && !readOnly && (
      ...
      <CompetitionTeamView roster={roster} assessments={assessments} config={config} selections={selections} attendance={attendance} filterCycle={filterCycle} filterGym={filterGym} onViewProfile={onViewProfile} />
...
function AppInner() {
  ...
  return (
    <div style={{ background: C.bg, color: C.text, minHeight: "100vh", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", WebkitFontSmoothing: "antialiased" }}>
      ...
      {/* Content */}
      {tab === "home" && <HomeScreen roster={roster} attendance={safeAttendance} assessments={approvedAssessments} config={safeConfig} selections={safeSelections} loggedCoach={loggedCoach} loggedGym={loggedGym} isAdmin={canToggleGyms} isCommunity={isCommunity} onNavigate={(target) => {
      ...
      {tab === "reports" && <ReportingScreen roster={roster} assessments={approvedAssessments} config={safeConfig} onViewProfile={viewProfile} onScore={isCommunity ? () => {} : (kidId) => { setTab("score"); }} readOnly={isCommunity} isAdmin={canToggleGyms} loggedGym={loggedGym} selections={safeSelections} attendance={safeAttendance} />}
