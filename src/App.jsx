...
function computePromoProjection(kid, attendance, config) {
  ...
  const classesForCurrentStripes = stripes * (rules.stripeClasses || 10);
  ...
  const gates = [
    ...
    { label: "Time at belt", labelZh: "腰带时间", current: months, required: rules.beltMonths || 9, unit: "mo", done: monthsNeeded <= 0 },
    { label: "Stripes", labelZh: "条纹", current: stripes, required: maxStripes, done: stripesNeeded <= 0 },
  ];

  // Projected date = latest of all three gates
  const dates = [classDate, monthDate, stripeDate].filter(Boolean);
...
function WeeklyAttendanceChart({ kidId, attendance }) {
  ...
  const chartH = 100;
  const barGap = 1;
  const barW = data.length > 0 ? Math.max(2, Math.min(12, (320 - data.length * barGap) / data.length)) : 6;
  ...
  // Show labels every N weeks depending on range
  const labelEvery = range === "1m" ? 1 : range === "3m" ? 2 : range === "6m" ? 4 : 8;
    ...
    <>
      ...
      <div style={s.card}>
        ...
        <div style={{ overflowX: "auto", paddingBottom: 4 }}>
          <svg width={Math.max(chartW, 280)} height={chartH + 20} style={{ display: "block" }}>
            {/* Grid lines */}
            {[0.25, 0.5, 0.75, 1].map(pct => (
              <line key={pct} x1={0} x2={chartW} y1={chartH * (1 - pct)} y2={chartH * (1 - pct)} stroke={C.border} strokeWidth={0.5} />
            ))}
            {/* Bars */}
            {data.map((w, i) => {
              ...
              return (
                <g key={i}>
                  <rect x={x} y={chartH - h} width={barW} height={Math.max(h, 0.5)} rx={1}
                    fill={w.count === 0 ? C.card2 : isCurrentWeek ? C.orange : C.red} opacity={w.count === 0 ? 0.4 : 0.85} />
                  {w.count > 0 && barW >= 8 && (
                    <text x={x + barW / 2} y={chartH - h - 3} textAnchor="middle" fontSize={8} fill={C.textDim}>{w.count}</text>
                  )}
                  {i % labelEvery === 0 && (
                    <text x={x + barW / 2} y={chartH + 12} textAnchor="middle" fontSize={8} fill={C.textMuted}>{formatWeek(w.start)}</text>
                  )}
                </g>
              );
            })}
            {/* Y-axis max label */}
            <text x={chartW + 4} y={8} fontSize={9} fill={C.textDim}>{maxCount}</text>
          </svg>
        </div>
      </div>
    </>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {kid && <button style={s.btnSm} onClick={() => {
            ...
            const attData = (() => {
              ...
              const d90 = new Date(); d90.setDate(d90.getDate() - 90);
              const cutoff90 = d90.toISOString().slice(0, 10);
              const att90 = kidAtt.filter(r => r.date >= cutoff90).length;
              const weeks90 = Math.max(1, Math.round(90 / 7));
              const weeklyAvg = (att90 / weeks90).toFixed(1);

              // Weekly chart — last 12 weeks as sparkline bars
              const getMonday = (d) => { const dt = new Date(d); const day = dt.getDay(); const diff = day === 0 ? -6 : 1 - day; dt.setDate(dt.getDate() + diff); dt.setHours(0,0,0,0); return dt; };
              const now = new Date();
              const wks = [];
              for (let i = 11; i >= 0; i--) { const m = getMonday(new Date(now.getTime() - i * 7 * 86400000)); wks.push({ start: m, count: 0 }); }
              kidAtt.forEach(r => {
              ...
              const sparkSvg = '<svg viewBox="0 0 ' + (12 * (bw + gap)) + ' ' + bh + '" style="width:100%;height:32px;margin-top:6px">' + bars + '</svg>';

              return { total, weeklyAvg, sparkSvg };
            })();
...
<div class="col-r">
  ...
  <div class="card">
    <div class="card-title">Attendance 出勤 (90 days)</div>
    ...
    <div style="margin-top:6px;font-size:8px;color:#aaa;text-align:center">Last 12 weeks · 最近12周</div>
    ${attData.sparkSvg}
  </div>
