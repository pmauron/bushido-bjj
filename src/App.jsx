  ...
  const loadDemoData = () => {
    ...
    for(let d=new Date(sD);d<=eD;d=ad(d,1)){
      ...
      if([1,3,5].includes(dow)){
        ...
        newAtt.push({key:`${dStr}|group|${DEMO_GYM}`,date:dStr,type:"group",gym:DEMO_GYM,records:rec});
      }
      if([3,6].includes(dow)){
        const rec={}; cKids.forEach(k=>{const r=Math.random();if(r<0.75)rec[k.id]="attend";else if(r<0.9)rec[k.id]="excused";});
        newAtt.push({key:`${dStr}|competition|${DEMO_GYM}`,date:dStr,type:"competition",gym:DEMO_GYM,records:rec});
      }
    }
    // Goals & Promotion Log
    const newGoals = {};
