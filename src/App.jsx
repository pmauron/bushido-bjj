...
const PAGE_HELP = {
  ...
  attendance: {
    en: "Track class attendance by date, type (group/competition), and gym. Tap a kid to cycle status: Absent → ✅ Present → 🟡 Excused. Home gym kids listed first; cross-training kids in a separate section below. Both sections have search bars and scroll. Admin and community members only.",
    zh: "按日期、类型（小组课/竞赛课）和道馆记录出勤。点击学员切换状态：缺席 → ✅ 出勤 → 🟡 请假。本馆学员优先显示，跨馆训练学员在下方单独列出。两个列表均可搜索和滚动。仅管理员和社区成员可见。",
  },
  promotion: {
...
/* ━━━ ROSTER SCREEN ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
function RosterScreen({ roster, setRoster, config, assessments, onViewProfile, defaultGym, isAdmin }) {
...
${(() => {
  ...
  const excused = kidAtt.filter(r => r.records[kid.id] === "excused").length;
  const absent = kidAtt.length - attend - excused;
  ...
  const rate = kidAtt.length ? (attend / kidAtt.length * 100).toFixed(0) : 0;
  return `<div class="section">
  <h2>Attendance Summary 出勤统计</h2>
  <table>
    <thead><tr><th>Metric 指标</th><th>Value 数值</th></tr></thead>
    <tbody>
    <tr><td>Total classes 总课数</td><td>${kidAtt.length}</td></tr>
    <tr><td>✅ Present 出勤</td><td>${attend}</td></tr>
    <tr><td>🟡 Excused 请假</td><td>${excused}</td></tr>
    <tr><td>— Absent 缺席</td><td>${absent}</td></tr>
    <tr><td>Attendance rate 出勤率</td><td><strong>${rate}%</strong></td></tr>
    <tr><td>👥 Group classes 小组课</td><td>${groupAtt}</td></tr>
    ${compAtt ? `<tr><td>🏆 Competition classes 竞赛课</td><td>${compAtt}</td></tr>` : ""}
    </tbody>
  </table></div>`;
})()}
...
/* ━━━ REPORTING SCREEN ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
function ReportingScreen({ roster, assessments, config, onViewProfile, onScore, readOnly, isAdmin, loggedGym }) {
  const [filterCycle, setFilterCycle] = useState(config.cycles.filter(c => isQuarterClosed(c)).slice(-1)[0] || config.cycles[0] || "");
  const [filterGym, setFilterGym] = useState(isAdmin ? "" : (loggedGym || ""));
...
function PromotionScreen({ roster, setRoster, attendance, config, setConfig, loggedCoach, isCommunity, isAdmin, loggedGym, onViewProfile }) {
  ...
  const eligibility = useMemo(() => {
    return roster.filter(k => k.active && kidInGym(k, gym)).map(kid => {
...
function UserGuide({ onClose }) {
  ...
  const sections = [
    ...
    { icon: "📋", title: "Attendance", titleZh: "出勤",
      ...
      visual: { type: "card", label: "Status Cycle", items: ["— Absent (default)", "✅ Present (tap once)", "🟡 Excused (tap twice)", "— Absent (tap again)"] },
    },
...
export default function App() {
  ...
  const promoCount = useMemo(() => {
    ...
    return roster.filter(k => k.active && (!gymFilter || kidInGym(k, gymFilter))).filter(kid => {
      const stripes = kid.stripes || 0;
      const lastPromo = kid.lastPromotionDate || "2020-01-01";
      ...
      const months = lastPromo ? Math.floor((Date.now() - new Date(lastPromo).getTime()) / (30.44 * 86400000)) : 999;
      const stripeReady = stripes < (rules.stripesForBelt || 4) && classes >= (rules.stripeClasses || 10);
      const beltReady = beltIdx < safeConfig.belts.length - 1 && stripes >= (rules.stripesForBelt || 4)
  ...
  return (
    <div style={{ background: C.bg, minHeight: "100vh", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", WebkitFontSmoothing: "antialiased" }}>
      ...
      {/* Content */}
      {tab === "roster" && <RosterScreen roster={roster} setRoster={setRoster} config={safeConfig} assessments={safeAssessments} onViewProfile={viewProfile} defaultGym={loggedGym} isAdmin={isAdmin} />}
      ...
      {tab === "reports" && <ReportingScreen roster={roster} assessments={safeAssessments} config={safeConfig} onViewProfile={viewProfile} onScore={isCommunity ? () => {} : (kidId) => { setTab("score"); }} readOnly={isCommunity} isAdmin={isAdmin} loggedGym={loggedGym} />}
