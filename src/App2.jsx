          ...
          <div style={s.card}>
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 10 }}>
              <input style={{ ...s.input, flex: 1 }} placeholder="New member name…" id="newCommunityInput"
                onKeyDown={e => { if (e.key === "Enter" && e.target.value.trim()) { setConfig(p => ({ ...p, communityMembers: [...(p.communityMembers || []), { name: e.target.value.trim(), gym: config.gyms[0] || "", pin: "bushido" }] })); e.target.value = ""; }}} />
              <button style={s.btn} onClick={() => { const inp = document.getElementById("newCommunityInput"); if (inp.value.trim()) { setConfig(p => ({ ...p, communityMembers: [...(p.communityMembers || []), { name: inp.value.trim(), gym: config.gyms[0] || "", pin: "bushido" }] })); inp.value = ""; }}}>Add</button>
            </div>
          </div>
        </div>
      )}
              ...
              return (
                <button key={key} onClick={() => { setSelectedCoach(key); setError(""); }} style={{
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: C.textDim }}>Community · {m.gym || "No gym"}</div>
                  </div>
                  {active && <span style={{ marginLeft: "auto", fontSize: 16 }}>✓</span>}
                </button>
              );
